...
ppn_num = 2    # number of processors per node demonstrated
...
# step 6: match pseudopotential labels with atomic coordinates
        for t in range(natoms):
            coord_counter = coordpturb_counter + coordatom_counter + coordmag_counter # counter shifts label read into output due to both perturbations and atomic species changes
            ...
            elif atoms_lib[j][t+1] != atoms_lib[j][t]:  # change solely in atomic species accounted for here, not magnetic state (from 1st column of .xyz file)
                coordatom_counter += 1   # possible error: 2 atoms of different species with same magnetic mome
            else:
                if coord_order[j][t+1] != coord_order[j][t]:
                    coordmag_counter += 1  # chages in magnetic state accounted for here (from comment line of .xyz file)

            if coord_order[j][t-1] != coord_order[j][t]:
                magonly_counter += 1

            magtyp_lib[t] = [coord_counter, t, magonly_counter]
...
#        coordmag_counter = 0
        ...
        for t in range(natoms):
            ...
            magonly_list.append(magtyp_lib[t][2])
...
#        print magonly_list
        if magchecker_setlength == 1:
            if netmag_counter == 0:
                ...
                for u in range( magchecker_setlength ):
            ...
            if netmag_counter == 0:
                ...
                for u in range(natoms):
                    for v in range(natoms):
                        if v == magtyp_lib[u][1] and coord_order[j][v] == 1:
                            starting_magnetization[u] = [magmoment[j][0], magtyp_lib[u][2], magtyp_lib[u][0]]
                        elif v == magtyp_lib[u][1] and coord_order[j][v] == -1:
                            starting_magnetization[u] = [magmoment[j][1], magtyp_lib[u][2], magtyp_lib[u][0]]
                        else:
                            pass
                ...
                for u in range( len( set(magtyp_list) ) ):
                    for v in range(natoms):
                        if v == magonly_lib[u][1] and coord_order[j][v] == 1:
                            starting_magnetization[u] = [magmoment[j][0], magtyp_lib[u][2]]
                        for w in range(ferrimag_mults):
                            if v == magonly_lib[u][1] and coord_order[j][v] == ferrimag_mults[w]:
                                starting_magnetization[u] = [magmoment[j][1], magtyp_lib[u][2]]
                        else:
                            pass
...
#            inputfile_alpha.write(' wfcdir = "./",\n')
            ...
            if magchecker_setlength < 2 and netmag_counter == 0:
                ...
                for n in range(8): #natoms-1):
                    if n == 0:
                        inputfile_alpha.write(' starting_magnetization({0}) = {1}\n'.format(starting_magnetization[n][2]+1, starting_magnetization[n][0]))
                    elif starting_magnetization[n-1][2] != starting_magnetization[n][2]:
                        inputfile_alpha.write(' starting_magnetization({0}) = {1}\n'.format(starting_magnetization[n][2]+1, starting_magnetization[n][0]))
...
#+RESULTS:
: {0: [0.5, 1, 0], 1: [0.5, 1, 1], 2: [0.5, 1, 1], 3: [0.5, 1, 1], 4: [-0.5, 2, 2], 5: [-0.5, 2, 2], 6: [-0.5, 2, 2], 7: [-0.5, 2, 2]}
: {0: [0.5, 1, 0], 1: [0.5, 1, 0], 2: [0.5, 1, 0], 3: [0.5, 1, 0], 4: [-0.5, 2, 1], 5: [-0.5, 2, 1], 6: [-0.5, 2, 1], 7: [-0.5, 2, 1]}
